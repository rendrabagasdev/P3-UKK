...
@section('title', 'Kelola Jadwal Ruangan')
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const bookingsEvents = [
      @foreach($rooms as $room)
        @foreach($room->bookings as $booking)
          @if(in_array($booking->status, ['proses','diterima','ditolak']))
          {
            id: {{ $booking->id_booking }},
            title: '{{ addslashes($room->nama_room) }}',
            start: '{{ $booking->tanggal_mulai }}',
            end: '{{ \Carbon\Carbon::parse($booking->tanggal_selesai)->addDay()->format('Y-m-d') }}',
            backgroundColor: @if($booking->status === 'proses') '#fbbf24' @elseif($booking->status === 'diterima') '#10b981' @else '#ef4444' @endif,
            borderColor: @if($booking->status === 'proses') '#f59e0b' @elseif($booking->status === 'diterima') '#059669' @else '#dc2626' @endif,
            extendedProps: {
              jenis: 'booking',
              status: '{{ $booking->status }}',
              roomId: {{ $room->id_room }},
              roomName: '{{ addslashes($room->nama_room) }}',
              description: '{{ addslashes($booking->keterangan ?? '-') }}',
    ...
    const regularEvents = [
      @foreach($rooms as $room)
        @foreach($room->jadwalRegulers as $reg)
          {
            id: 'R{{ $reg->id_reguler }}',
            title: '{{ addslashes($room->nama_room) }} (Reguler)',
            start: '{{ $reg->tanggal_mulai }}',
            end: '{{ \Carbon\Carbon::parse($reg->tanggal_selesai)->addDay()->format('Y-m-d') }}',
            ...
            extendedProps: {
              ...
              roomId: {{ $room->id_room }},
              roomName: '{{ addslashes($room->nama_room) }}',
              description: '{{ addslashes($reg->keterangan ?: 'Jadwal Reguler') }}',
              endDate: '{{ $reg->tanggal_selesai }}'
            }
          },
        @endforeach
      @endforeach
    ...
    function updateRoomChips() {
      ...
      @foreach($rooms as $room)
        (function(){
          ...
          chip.innerHTML = `<span>{{ addslashes($room->nama_room) }}</span><span class="text-xs text-gray-500">(${c.total})</span>`;
