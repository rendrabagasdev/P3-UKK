    ...
    <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-amber-500">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-3xl font-bold text-gray-900 mt-1">{{ $bookings->where('status', 'proses')->count() }}</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-emerald-500">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-3xl font-bold text-gray-900 mt-1">{{ $bookings->where('status', 'diterima')->count() }}</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-rose-500">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-3xl font-bold text-gray-900 mt-1">{{ $bookings->where('status', 'ditolak')->count() }}</p>
        </div>
  ...
  <div class="bg-white rounded-xl shadow-md p-6">
    <form method="GET" class="flex flex-wrap gap-4 items-end">
      ...
      <div class="flex-1 min-w-[200px]">
        ...
        <input type="date" name="dari" value="{{ request('dari') }}" class="w-full border-2 border-gray-300 rounded-lg px-4 py-2 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200" />
      </div>
      <div class="flex-1 min-w-[200px]">
        <label class="block text-sm font-semibold text-gray-700 mb-2">
          <i class="fas fa-calendar-check text-indigo-600 mr-2"></i>Sampai Tanggal
        </label>
        <input type="date" name="sampai" value="{{ request('sampai') }}" class="w-full border-2 border-gray-300 rounded-lg px-4 py-2 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200" />
      </div>
      <div>
        ...
        <a href="{{ route('user.bookings.history') }}" class="ml-2 px-6 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 font-semibold shadow-md hover:shadow-lg transition">
          ...
          <tr>
            <td colspan="5" class="px-6 pt-4">
              <div class="flex flex-wrap items-center gap-2 text-sm">
                ...
                @if(request('status'))
                  <span class="px-2.5 py-1 rounded-full bg-gray-100 text-gray-700">Status: {{ ucfirst(request('status')) }}</span>
                @endif
                @if(request('dari'))
                  <span class="px-2.5 py-1 rounded-full bg-gray-100 text-gray-700">Dari: {{ request('dari') }}</span>
                @endif
                @if(request('sampai'))
                  <span class="px-2.5 py-1 rounded-full bg-gray-100 text-gray-700">Sampai: {{ request('sampai') }}</span>
                @endif
              </div>
            </td>
          </tr>
          ...
          <tr class="hover:bg-gray-50 transition">
            <td class="px-6 py-4">
              <div class="flex items-center">
                ...
                <span class="text-base font-semibold text-gray-900">{{ $booking->room->nama_room ?? '-' }}</span>
              </div>
            </td>
            <td class="px-6 py-4">
              <div class="text-base text-gray-900">
                ...
                @if(($booking->tipe_booking ?? 'hourly') === 'hourly')
                  <div class="font-semibold">{{ $start->translatedFormat('d M Y') }}</div>
                  <div class="text-sm text-gray-500">{{ $start->format('H:i') }} – {{ $end->format('H:i') }} ({{ $end->diffInHours($start) }} jam)</div>
                @else
                  <div class="font-semibold">{{ $start->translatedFormat('d M Y') }} s/d {{ $end->translatedFormat('d M Y') }}</div>
                  <div class="text-sm text-gray-500">Durasi {{ $end->diffInDays($start)+1 }} hari</div>
                @endif
              </div>
            </td>
            <td class="px-6 py-4">
              <span class="text-base text-gray-700">{{ \Illuminate\Support\Str::limit($booking->keterangan, 50) }}</span>
            </td>
            <td class="px-6 py-4">
              ...
              @else
                <span class="px-4 py-2 text-sm font-bold rounded-full bg-gray-100 text-gray-800">{{ ucfirst($booking->status) }}</span>
              @endif
            </td>
            <td class="px-6 py-4 text-center">
              <div class="mb-2 text-sm text-gray-600">Tipe: {{ ($booking->tipe_booking ?? 'hourly')==='daily' ? 'Per Hari' : 'Per Jam' }}</div>
              @if($booking->status=='proses')
              <form method="POST" action="{{ route('user.bookings.cancel', $booking->id_booking) }}" onsubmit="return confirm('Yakin ingin membatalkan peminjaman ini?');">
                @csrf
                @method('DELETE')
              ...
              @elseif($booking->status=='ditolak' && $booking->alasan_tolak)
              <button onclick="alert('Alasan: {{ $booking->alasan_tolak }}')" class="px-4 py-2 text-sm font-bold bg-gray-500 hover:bg-gray-600 text-white rounded-lg shadow-md hover:shadow-lg transition-all">
