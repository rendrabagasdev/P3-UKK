...
<div x-data="bookingForm()" class="max-w-5xl mx-auto space-y-6">
  ...
  <div class="flex justify-between bg-gray-50 rounded-lg px-3 py-2"><span class="text-gray-600">Ruangan:</span><span class="font-semibold">{{ $room->nama_room }}</span></div>
        <div class="flex justify-between bg-gray-50 rounded-lg px-3 py-2"><span class="text-gray-600">Tanggal:</span><span class="font-semibold">{{ \Carbon\Carbon::parse($tanggal)->translatedFormat('d M Y') }}</span></div>
      </div>
      <div class="space-y-2">
        <div class="flex justify-between bg-gray-50 rounded-lg px-3 py-2"><span class="text-gray-600">Waktu:</span><span class="font-semibold">{{ $jam_mulai }} - {{ $jam_selesai }}</span></div>
        <div class="flex justify-between bg-gray-50 rounded-lg px-3 py-2"><span class="text-gray-600">Durasi:</span><span class="font-semibold">{{ $durasi }} jam</span></div>
      </div>
    </div>
  </div>

  <form method="POST" action="{{ route('user.slot-booking.store') }}" class="space-y-6">
    @csrf
    ...
    <div class="bg-white rounded-xl shadow p-6 space-y-4">
      ...
      <div class="grid md:grid-cols-2 gap-3 text-sm">
        ...
        <div>
          ...
          <input type="text" class="w-full border rounded-lg px-3 py-2" value="{{ Auth::user()->nama ?? Auth::user()->username }}" readonly>
        </div>
      </div>
      <div>
  ...
  <!-- Catatan: Tidak ada fitur pembayaran pada sistem ini -->
    ...
    <div class="flex justify-between items-center">
      <a href="{{ route('user.slot-booking.show', $room->id_room) }}" class="px-5 py-2 rounded-lg bg-gray-200 text-gray-700 text-sm font-medium hover:bg-gray-300 transition flex items-center gap-2"><i class="fas fa-arrow-left"></i> Kembali</a>
